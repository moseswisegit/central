...
class FilesController extends Controller
{
    ...
    public function index()
    {
        ...
        $enteteContent = "Fichiers";


        return view('superAdmin.files.index', compact('files', 'userFilesCount','enteteContent'));
    }
    ...
    public function create()
    {
        ...
        if ($roleId == 2 && $userFilesCount > 5) {
            return redirect('/superAdmin/files');
        }

        $folders = \App\Folder::get()->pluck('name', 'id')->prepend(trans('quickadmin.qa_please_select'), '');
        $created_bies = \App\User::get()->pluck('name', 'id')->prepend(trans('quickadmin.qa_please_select'), '');

        return view('superAdmin.files.create', compact('folders', 'created_bies', 'userFilesCount', 'roleId'));
    }
